defmodule Kaizen.EvaluationTimeoutTest do
  use ExUnit.Case, async: true

  alias Kaizen.{Config, Engine, State}
  ...
  describe "timeout behavior in evolution" do
    test "fast fitness evaluations complete successfully" do
      ...
      states =
        initial_pop
        |> Engine.evolve(config, SlowFitness, Kaizen.Evolvable.String, context: %{delay: 10})
        |> Enum.take(2)
    ...
    test "slow fitness evaluations timeout and are killed" do
      ...
      states =
        initial_pop
        |> Engine.evolve(config, SlowFitness, Kaizen.Evolvable.String, context: %{delay: 500})
        |> Enum.take(1)
    ...
    test ":infinity timeout allows long-running evaluations" do
      ...
      states =
        initial_pop
        |> Engine.evolve(config, SlowFitness, Kaizen.Evolvable.String, context: %{delay: 200})
        |> Enum.take(1)
    ...
    test "timeout configuration affects Task.async_stream" do
      ...
      result =
        Engine.evolution_step(
          ...
          Kaizen.Mutation.Text,
          Kaizen.Selection.Tournament,
          Kaizen.Crossover.String,
  ...
  describe "timeout with different population sizes" do
    test "handles timeout with small population" do
      ...
      states =
        initial_pop
        |> Engine.evolve(config, SlowFitness, Kaizen.Evolvable.String, context: %{})
        |> Enum.take(1)
